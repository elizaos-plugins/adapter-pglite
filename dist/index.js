...
var PGLiteDatabaseAdapter = class extends DatabaseAdapter {
  constructor(options) {
    super();
    this.db = new PGlite({
      ...options,
    ...
    return this.withDatabase(async () => {
      try {
        const sql = `
                    ...
                    ORDER BY a.name
                `;
        const result = await this.query(sql, [params.roomId]);
    ...
    return this.withDatabase(async () => {
      try {
        ...
        return rows.map((row) => ({
          ...
          createdAt: row.createdAt ? new Date(row.createdAt).getTime() : void 0
        }));
      } catch (error) {
